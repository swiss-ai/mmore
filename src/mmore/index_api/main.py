...
async def upload_files(
    ...
    try:
        ...
        if len(listIds) != len(files):
            raise HTTPException(status_code=400, detail=f"Number of IDs ({len(listIds)}) doesn't match number of files ({len(files)})")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            return {
                ...
                "documents": [{"id": doc.id, "text": doc.text[:50] + "..."} for doc in modified_documents]
            }
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                logger.warning(f"Error deleting existing document (may not exist): {str(delete_error)}")
...
async def delete_file(
    id: str = Path(..., description="ID of the file to delete")
):
    """
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Error deleting from vector DB (continuing): {str(db_error)}")
...
def create_new_indexer(collection_name: str, uri: str, db_name: str) -> Indexer:
    ...
    try:
        ...
        logging.info(f"Successfully created new indexer for collection: {collection_name}")
        return indexer
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create new indexer: {str(e)}"
        )
