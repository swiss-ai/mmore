...
from mmore.colpali.retriever import ColPaliRetriever, ColPaliRetrieverConfig, load_text_mapping
...
def test_pdf_converter_convert_to_pngs():
    ...
    try:
        ...
        assert all(p.exists() for p in png_paths), "All generated PNG files should exist"
        assert all(p.suffix == ".png" for p in png_paths), "All generated files should have .png extension"
    finally:
        converter.cleanup()
...
def test_colpali_embedder_init():
    ...
    run_process.ColPaliProcessor.from_pretrained = lambda *args, **kwargs: type('obj', (object,), {})()
...
def test_process_single_pdf_success():
    ...
    converter = PDFConverter()
    converter.convert_to_pngs = lambda pdf_path: [Path("page_1.png"), Path("page_2.png")]

    # Override embed_images to bypass model loading
    mock_embedder = type('obj', (object,), {
    ...
    try:
        page_records, text_records = process_single_pdf(Path(sample_file), mock_embedder, converter)

        assert len(page_records) == 2, "Should process 2 pages"
        assert len(text_records) == 2, "Should extract text from 2 pages"
        assert page_records[0]["pdf_path"] == str(Path(sample_file)), "PDF path should be correct"
        assert page_records[0]["page_number"] == 1, "Page number should be 1"
        assert "embedding" in page_records[0], "Should include embedding"
        assert text_records[0]["text"] == "Page 1 text", "Text should be extracted correctly"
    finally:
        converter.cleanup()
...
def test_milvus_colpali_manager_init_error():
    ...
    mock_client_instance = type('obj', (object,), {
...
def test_milvus_colpali_manager_create_collection():
    ...
    mock_schema = type('obj', (object,), {
        'add_field': lambda self, *args, **kwargs: None
    })()
    
    mock_client_instance = type('obj', (object,), {
        'has_collection': lambda self, name: False,
        'create_schema': lambda self, **kwargs: mock_schema,
        'create_collection': lambda self, **kwargs: create_collection_called.append(True),
    })()
    ...
    try:
        ...
        assert len(create_collection_called) > 0, "Should call create_collection when create_collection=True"
    finally:
        milvuscolpali.MilvusClient = original_milvus_client
...
def test_milvus_colpali_manager_insert_from_dataframe():
    ...
    try:
        ...
        assert len(insert_calls) == 2, "Should insert 2 batches for 2 rows with batch_size=1"
    finally:
        milvuscolpali.MilvusClient = original_milvus_client
...
def test_colpali_retriever_from_config():
    ...
    from mmore.colpali import retriever, milvuscolpali
    ...
    try:
        ...
        assert isinstance(retriever_instance.manager, MilvusColpaliManager), "Manager should be MilvusColpaliManager instance"
